...
export const useSelect = ({isDisabled = false, state, onFetchMore, itemsToPrefetch = 3}: UseSelectProps) => {
	useInput(
		(_input, key) => {
			if (key.downArrow) {
				...
				if (onFetchMore && state.focusedValue) {
					...
					if (currentOption && currentOption.index >= state.optionMap.size - itemsToPrefetch) {
						onFetchMore();
					}
				}
			}
